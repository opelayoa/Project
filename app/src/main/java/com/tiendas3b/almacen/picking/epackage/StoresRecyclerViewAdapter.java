...
public class StoresRecyclerViewAdapter extends BaseRecyclerViewAdapter<Store> {
    ...
    protected void bindView(Store item, ViewHolder viewHolder) {
        if (item != null) {
            ...
            lblQuantityPackages.setText(String.valueOf(((db.listEPackagesStore(id, EpackageActivity.PICKING_CAGE_IN, dateStr)) == null ? 0 :(db.listEPackagesStore(id, EpackageActivity.PICKING_CAGE_IN, dateStr).size()))));
        }
    }

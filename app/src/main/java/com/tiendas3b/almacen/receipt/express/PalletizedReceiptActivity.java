...
import com.tiendas3b.almacen.receipt.palletized.PalletizedReceiptRvAdapter;
import com.tiendas3b.almacen.receipt.palletized.VArticlePalletized;
...
public class PalletizedReceiptActivity extends AppCompatActivity {
    ...
    EditText txtBarcode;
    private PalletizedReceiptRvAdapter adapter;
    private long buyId;
    private int iclave;
...
//            btnSend.setEnabled(true);
    ...
    private void createExpressReceipt() {
        ...
        List<VArticlePalletized> list = adapter.getList();
        for (VArticlePalletized a : list) {
            BuyDetail det = db.findBuyDetailByArticle((int) a.getArticle().getIclave(), buyId);
            ...
            if (capture == null) {
                ...
                capture.setReceivedAmount(piecesForScaffold * count);
                capture.setPaletizadoCount(count);
...
//        receiptSheetCapture.setSerie(header.getSerie());
    ...
    private void setUpViews() {
        ...
        adapter = new PalletizedReceiptRvAdapter(mContext, null);
    ...
    private void updateList() {
        ...
        List<VArticlePalletized> palletizedReceipts = new ArrayList<>();

        for (ExpressReceipt er : expressReceipts) {
            boolean add = true;
            for (VArticlePalletized pr : palletizedReceipts) {
                if(er.getIclave() == pr.getArticle().getIclave()){
            ...
            if(add){
                VArticlePalletized ap = new VArticlePalletized();
                ap.setArticle(er.getBuyDetail().getArticle());
                ap.setTotal(1);
                ap.setCount(0);
                palletizedReceipts.add(ap);
            }
        }

        adapter.setListAndNotify(palletizedReceipts);

 ...
 */
public class StoresDownloadRecyclerViewAdapter extends BaseRecyclerViewAdapter<Store> {

    private IDatabaseManager db;
    private String dateStr;

    public StoresDownloadRecyclerViewAdapter(Context context, IDatabaseManager datbase, String date, OnViewHolderClick listener) {
    ...
    protected void bindView(Store item, ViewHolder viewHolder) {
        if (item != null) {
            ...
            lblQuantityPackages.setText(String.valueOf(((db.listEPackagesStore(id, EpackageActivity.SHIPMENT_RECEIVED, dateStr)) == null ? 0 :(db.listEPackagesStore(id, EpackageActivity.SHIPMENT_RECEIVED, dateStr).size()))));
        }
    }

...
public class BuyDetailFragment extends Fragment {
    ...
    private static final String TAG = BuyDetailFragment.class.getSimpleName();
    ...
    private BuyDetailRecyclerViewAdapter adapter;
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        this.menu = menu;
        inflater.inflate(R.menu.menu_buy_detail, menu);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            case R.id.action_new:
                startActivityForResult(new Intent(mContext, ReceiptSheetHeaderCaptureActivity.class)
                        .putExtra(ReceiptSheetHeaderCaptureActivity.ODC, this.item.getId()), BuyDetailActivity.REQUEST_CODE);
                return true;
    ...
    private void send(Callback callback) {
        ...
        if (valid()) {
            ...
            httpService.validateReceiptSheet(item.getFolio(), item.getRegionId(), item.getProviderId(), DateUtil.getDateStr(item.getProgramedDate())).enqueue(new Callback<GeneralResponseDTO>() {
                @Override
...
////                swipeRefresh.setRefreshing(false);
//                TextView txt = (TextView) viewSwitcher.findViewById(R.id.txtEmpty);
...
//                viewSwitcher.setDisplayedChild(1);
    ...
    private void process(String folio, String billRef, String names) {
        Toast.makeText(mContext, "Procesado", Toast.LENGTH_LONG).show();
...
//        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View rootView = inflater.inflate(R.layout.fragment_buy_detail_list, container, false);
...
//        btnFinish.setEnabled(true);
        ...
        setItems();
//        adapter = new BuyDetailRecyclerViewAdapter(getActivity(), items, mListener, complaints, checks);
...
//            showSend();
    ...
    private void downloadPrintAGR() {

        Tiendas3bClient httpService = mContext.getHttpServiceWithAuth();
        httpService.getHojaReciboAGR(item.getFolio(), String.valueOf(mContext.getRegion()), item.getProviderId())
                .enqueue(new Callback<List<FhojaReciboDetTempDTO>>() {
                    @Override
                    public void onResponse(Call<List<FhojaReciboDetTempDTO>> call, Response<List<FhojaReciboDetTempDTO>> response) {

                        if (response.isSuccessful()) {
                            printTicket(response.body());
                        }
                    }

                    @Override
                    public void onFailure(Call<List<FhojaReciboDetTempDTO>> call, Throwable t) {
                        Log.e(TAG, "Error getHojaReciboAGR");
                    }
                });
    }

    private void printTicket(List<FhojaReciboDetTempDTO> expressReceipts) {
        ...
        InsertReceiptExpressLog insertReceiptExpressLog = new InsertReceiptExpressLog(databaseManager, mContext);
        insertReceiptExpressLog.insertRecExprLog(item, Constants.IDLOG_PRINT_TICKET, Constants.LOG_PRINT_TICKET);
    ...
    private void printTsc(List<FhojaReciboDetTempDTO> expressReceipts, String address) {
...
//                .append("\nProveedor: ").append(lblProviderName.getText()).append("\n").append(DateUtil.getTodayStr()).append("\nODC: ").append(odc).append("\n\n");
        HashMap<Integer, TicketAGR> sumHash = new HashMap<>();
        Log.d(TAG, "printTsc address:" + address);
//        for (FhojaReciboDetTempDTO r : expressReceipts) {
//            Integer key = r.getIclave();
//            if (sumHash.containsKey(key)) {
//                sumHash.get(key).setTotal(sumHash.get(key).getTotal() + 1);
//            } else {
//                sumHash.put(key, new TicketAGR(1, key, r.getBuyDetail().getArticle().getDescription()));
//            }
//        }
...
//        ticketDetails.append(tiendas3B).append("\n");
        String almacen = "Almacen:      " + databaseManager.getById(mContext.getRegion(), Region.class).getName();
//        ticketDetails.append(almacen).append("\n");
        String proveedor = "Proveedor:    " + item.getProvider().getName();
//        ticketDetails.append(proveedor).append("\n");
        String ticketOdc = "ODC:          " + item.getFolio();
...
//            }
            int granTotal = 0;
            for (int i = 0; i < expressReceipts.size(); i++) {
                int total = expressReceipts.get(i).getNum_pallets();
                VArticle vArticle = databaseManager.findViewArticleByIclave(expressReceipts.get(i).getIclave());

                granTotal += total;
                printer.text(y += 30, total + " PLL " + vArticle.getDescription());
            }
    ...
    class TicketAGR {
        ...
        public TicketAGR(int total, Integer key, String description) {
            this.total = total;
            iclave = key;
            desc = description;
        }
    ...
    public interface OnFragmentInteractionListener {
        ...
        void onFragmentInteraction(BuyDetailRecyclerViewAdapter.BuyDetailHolder holder, int position);

        void showCalendar(EditText txtExpiryDate, int extraDays);

        boolean onSaveDetail(BuyDetailRecyclerViewAdapter.BuyDetailHolder holder);

...
/**
 * Created by dfa on 20/04/2016.
 */
public class WeekView extends View {

    private static final int MAX_TIME = 14;//fix for init time
    ...
    private int mMaxHourHeight = 500;//250
...
//                    break;
                case VERTICAL:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * MAX_TIME + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight()), 0);
                    break;
            }
    ...
    private void init() {
        ...
        // Prepare hour thousandsSeparator color paint.
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        for (int i = 0; i < MAX_TIME; i++) {
            ...
            // Draw the textAndPrint if its y position is not outside of the visible area. The pivot point of the textAndPrint is the point at the bottom-right corner.
            String time = getDateTimeInterpreter().interpretTime(i);
            if (time == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null time");
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight= Math.max(mMinHourHeight, (int) ((getHeight() - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / MAX_TIME));
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * MAX_TIME - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2)
            mCurrentOrigin.y = getHeight() - mHourHeight * MAX_TIME - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2;
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            // Prepare the thousandsSeparator lines for hours.
            int i = 0;
            for (int hourNumber = 0; hourNumber < MAX_TIME; hourNumber++) {
...
//            leftDays = Math.ceil(leftDays);
    ...
    public void goToHour(double hour){
        ...
        if (hour > MAX_TIME)
            verticalOffset = mHourHeight * MAX_TIME;
        else if (hour > 0)
            verticalOffset = (int) (mHourHeight * hour);

        if (verticalOffset > mHourHeight * MAX_TIME - getHeight() + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int)(mHourHeight * MAX_TIME - getHeight() + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom);

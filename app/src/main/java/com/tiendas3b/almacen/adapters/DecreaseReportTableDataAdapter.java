...
public class DecreaseReportTableDataAdapter extends TableDataAdapter<ArticleVO> {

    private static final int TEXT_SIZE = 14;
    private final Context mContext;
    private List<ArticleVO> items;

    public DecreaseReportTableDataAdapter(Context context, List<ArticleVO> data) {
        super(context, data);
        mContext = context;
        items = data;
    }
        ...
        if(rowIndex < getCount() - 1) {
            switch (columnIndex) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    renderedView = renderString(String.valueOf(item.getAmount()), Gravity.END);
                    break;
                case 4:
                    renderedView = renderString(String.valueOf(item.getCost()), Gravity.END);
                    break;
                case 5:
                    renderedView = renderString(item.getObsStr(), Gravity.CENTER_HORIZONTAL);
                    break;
            }
        } else {
            switch (columnIndex) {
